...
template <Matrix A, Matrix B> lazy_add<A, B> operator+(A const &a, B const &b) { return {a, b}; }
...
template <typename F, Matrix A> lazy_call<F, A> make_lazy_call(F f, A const &a) { return {f, a}; }

template <Matrix A> auto abs(A const &a) {
  return make_lazy_call(
     [](auto const &x) {
       using std::abs;
       return abs(x);
     },
